...
class SetupCallback(Callback):
    ...
    def __init__(self, resume, now, logdir, ckptdir, cfgdir, config, lightning_config):
        ...
        self.now = now  # 保存参数 now，通常用于表示当前时间戳或日期
        self.logdir = logdir  # 保存参数 logdir，日志文件的目录
        self.ckptdir = ckptdir  # 保存参数 ckptdir，检查点文件的目录
        self.cfgdir = cfgdir  # 保存参数 cfgdir，配置文件的目录
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0:  # 检查当前进程是否为全局 rank 为 0 的进程（通常是主进程）
            # 创建日志目录和保存配置
            os.makedirs(self.logdir, exist_ok=True)  # 创建日志目录，如果目录已存在则不会报错
            os.makedirs(self.ckptdir, exist_ok=True)  # 创建检查点目录，如果目录已存在则不会报错
            os.makedirs(self.cfgdir, exist_ok=True)  # 创建配置目录，如果目录已存在则不会报错
...
class ImageLogger(Callback):
    ...
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
        ...
        super().__init__()  # 调用父类的初始化方法
        self.rescale = rescale  # 是否重新缩放图像
        self.batch_freq = batch_frequency  # 日志记录的批次频率
        self.max_images = max_images  # 最大图像数量
        ...
        if not increase_log_steps:  # 如果不增加日志记录步数
            self.log_steps = [self.batch_freq]  # 将日志记录步数设置为批次频率
        self.clamp = clamp  # 是否启用限制
        self.disabled = disabled  # 是否禁用日志记录
        self.log_on_batch_idx = log_on_batch_idx  # 是否在批次索引上记录日志
        self.log_images_kwargs = log_images_kwargs if log_images_kwargs else {}  # 日志图像的额外参数，如果未提供则为空字典
        self.log_first_step = log_first_step  # 是否记录第一个步骤
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
        ...
        self.num_workers = num_workers if num_workers is not None else batch_size * 2  # 设置工作线程数，如果未指定则为批量大小的两倍
        self.use_worker_init_fn = use_worker_init_fn  # 是否使用worker初始化函数
    ...
    def setup(self, stage=None):
        self.datasets = dict(  # 初始化数据集字典
            (k, instantiate_from_config(self.dataset_configs[k]))  # 根据数据集配置字典中的配置实例化数据集，并存入数据集字典
    ...
    def _val_dataloader(self, shuffle=False):
        if isinstance(self.datasets['validation'], Txt2ImgIterableBaseDataset) or self.use_worker_init_fn:  # 检查验证数据集是否为可迭代的数据集或是否需要使用worker初始化函数
            ...
            init_fn = None  # 否则不设置初始化函数
        return DataLoader(self.datasets["validation"],  # 返回验证数据加载器
                          batch_size=self.batch_size,  # 设置批量大小
                          num_workers=self.num_workers,  # 设置工作线程数
                          worker_init_fn=init_fn,  # 设置worker初始化函数
                          shuffle=shuffle)  # 根据传入的参数决定是否打乱数据
...
if __name__ == "__main__":
    ...
    try:
        ...
        for k in nondefault_trainer_args(opt):  # 遍历opt中非默认的trainer参数
            trainer_config[k] = getattr(opt, k)  # 将这些参数的值更新到trainer_config中
        ...
        default_logger_cfgs = {  # 创建一个字典default_logger_cfgs，包含日志记录器的配置
            "wandb": {  # 配置WandbLogger
                ...
                "params": {  # 参数配置
                    "name": nowname,  # 日志记录器的名称
                    "save_dir": logdir,  # 保存日志的目录
                    "offline": opt.debug,  # 是否离线模式，取决于opt.debug
                    "id": nowname,  # 日志记录器的ID
                }
            },
        ...
        if hasattr(model, "monitor"):  # 如果model对象有"monitor"属性
            print(f"Monitoring {model.monitor} as checkpoint metric.")  # 打印监控的检查点指标
        ...
        for k in data.datasets:  # 遍历 data.datasets 字典中的每一个键 k
        ...
        # 打印累积梯度的批次数
        print(f"accumulate_grad_batches = {accumulate_grad_batches}") #2
        ...
        def melk(*args, **kwargs):  # 定义一个名为 melk 的函数，接受任意数量的参数和关键字参数
            ...
            if trainer.global_rank == 0:  # 如果当前节点是主节点（global_rank 为 0）
                ...
                ckpt_path = os.path.join(ckptdir, "512_vae.ckpt")  # 定义检查点保存路径，将其命名为 "last.ckpt"
